...
| `ccs watch` | Live usage dashboard | `--watch` |
| `ccs disable <num\|email>` | Hold an account out of rotation | `--disable-account` |
| `ccs enable <num\|email>` | Put it back in rotation | `--enable-account` |
| `ccs auto` | Auto-switch by usage % | — |
| `ccs upgrade` (`update`) | Self-upgrade to latest | `--upgrade` |
| `ccs purge` | Remove all claude-swap data | `--purge` |

Auto-switching is `ccs auto` — see [Automatic switching](#automatic-switching) below.
